...
class ParseThread(BaseThread):
    ...
    def working(self):
        ...
        # ----2----
        parse_state, url_list, save_list,content_dict = self._worker.working(priority, url, keys, deep, content)
        # ----3----
        if parse_state > 0:
            ...
            #for jobblog the len(save_list)==1
            self._pool.add_a_task(TPEnum.ITEM_SAVE, (url, keys, save_list[0],content_dict))
        else:
